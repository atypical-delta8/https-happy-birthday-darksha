document.addEventListener('DOMContentLoaded', () => {
    ...
    const giftOptionButtons = document.querySelectorAll('.gift-option-button');
    const optionContinueButton = document.getElementById('optionContinueButton');
    const selectedOptionText = document.getElementById('selectedOptionText');
    const finalGreetingElement = document.getElementById('finalGreeting');

    let selectedGiftOption = null;
    ...
    // --- Step 3: Gift Option Selection ---
    giftOptionButtons.forEach(button => {
        button.addEventListener('click', () => {
            giftOptionButtons.forEach(btn => btn.classList.remove('selected'));
            button.classList.add('selected');
            selectedGiftOption = button.dataset.option;

            const optionMessages = {
                'if-u': 'You chose the heartfelt route — a sweet surprise is coming your way.',
                'travel': 'Travel plans are in motion — get ready for an adventure-filled birthday.',
                'or-just-be': 'Sometimes the best gift is just being together — that path is chosen.'
            };

            selectedOptionText.textContent = optionMessages[selectedGiftOption] || 'Pick one and your surprise will be revealed.';
            optionContinueButton.classList.remove('hidden');
        });
    });

    optionContinueButton.addEventListener('click', () => {
        if (!selectedGiftOption) return;
